...
model = LSTM(word_dim=len(word_cnt), word_embed_dim=256, tag_dim=len(pos_cnt), hidden_dim=256, max_norm=1, use_gpu=True)
...
train_with_sgd(model, x1, y1, learning_rate=0.005, nepoch=10, evaluation_loss_after=1)
...
model.save('../weight/lstm_model_256_cpu.pkl')
...
save_data(data_to_save, '../weight/vocab_data_lstm_f32_256_cpu.pkl')
